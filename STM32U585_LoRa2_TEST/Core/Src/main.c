...
uint8_t SetStandby[] 			= {0x0A, 0x00};  //STDBY_RC = 0; STDBY_XOSC = 1
uint8_t SetPacketType[] 		= {0x8A, 0x01}; //PACKET_TYPE_LoRa 0x01 LoRa mode
uint8_t SetModulationParams[] 	= {0x8B, 0x0C, 0x04, 0x04, 0x00}; //0x07==SF7 | 0x04==125kHz | 0x01==LoRa_CR_4_5
uint8_t SetPacketParams[] 		= {0x8C, 0x00, 0x0C, 0x00, 0x05, 0x01, 0x00}; //Preamble MSB=0x00 LSB=0x0C (12) | Header-0x00 | Len=0x05 | CRC=0x01(on) | IQ=0x00(std)
uint8_t SetTxParams[] 			= {0x8E, 0x0E, 0x04};  //0x0E==14dBm | 0x04==SET_RAMP_200U
...
uint8_t SetDioIrqParams[]       = {0x08, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00};  //x
...
void lora_command(uint8_t *cmd, uint8_t cmd_len){
	...
	tsize   = cmd_len;  //tsize limit < 65535
	command = cmd;
	//set spe = 1
	ASM_SPI_CR1_SPE_0();
	...
	for(volatile int i=0; i<100; i++);	//PAUSE
...
void SPI1_IRQHandler(){
	...
	if(ASM_SPI_SR_Get() & (0x1U << 3)){
		//printf("EOT: end of transfer. 1: transfer complete.\n");
		ASM_SPI_IFCR_EOTC_Clear();
		for(volatile int i=0; i<200; i++);	//PAUSE
	...
	if(ASM_SPI_SR_Get() & (0x1U << 1)){
		//printf("TXP: Data packet space available\n");
	...
	while(ASM_SPI_SR_Get() & (0x1U)){
		//printf("RXP: Rx-packet available. 1: RxFIFO contains at least one data packet\n");
	...
	if(ASM_SPI_SR_Get() & (0x1U << 4)){
		//printf("TXTF: transmission transfer filled. 1: TxFIFO upload is finished.\n");
		ASM_SPI_IFCR_TXTFC();
	}
}

void EXTI15_IRQHTandler(){
